...
import Birdie from './Images/StressBirdie.png';
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="flex justify-center">
          <button className="clean-image-button" onClick={() => goToHome()}>
            <img src={Birdie} alt="Logo" className="w-15 h-15" />
          </button>
        </div>
      </div>
      ...
      <Card className="flex flex-col h-full bg-[#F2F5FA] shadow-lg p-6 rounded-lg" style={{ height: '700px' }}>
        <div className="messages-container flex-1 overflow-y-auto flex flex-col gap-4 mb-4" style={{ maxHeight: 'calc(100% - 100px)', overflowY: 'auto' }}>
          {messages.map((msg, index) => (
            <div key={index} className={`bubble ${msg.isUser ? 'bg-[#ff596a] text-white ml-auto rounded-lg p-2' : 'bg-gray-300 text-black mr-auto rounded-lg p-2'}`}>
              {msg.isUser ? (
                msg.text
              ) : (
        ...
        <div className="input-container flex text-white items-center bg-[#ff596a] p-3 rounded-2xl shadow-md mb-6 gap-2" style={{ height: '70px' }}>
          <input
            type="text"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={handleKeyPress}
            className="flex-1 p-3 bg-[#B62121] text-white rounded-xl outline-none"
          />
          <button onClick={handleSend} className="p-3 bg-[#B62121] text-white rounded-xl hover:bg-[#ff596a]">
            Enter
          </button>
        </div>
      </Card>
